...
class class_train_model():    
    ...
    def tokenizacao(self,df):

        # cria a coluna com textos vetorizados
        rows, cols = df.shape

        df['token'] = [self.preprocess_text(df["COMMENT_TEXT"][row]) for row in range(rows)]

        return df


    def treinar_vetorizacao(self,df):
    ...
    def vectorize2(self,lista):
        
        lista2 = [self.vectorize.vocabulary_[item] for item in lista]

        return lista2
    
    
    def vectorize3(self,lista, vectorize):
    ...
    def separa_df_test_train(self,df, size):
    ...
    def redes_neurais_modelo(self,lista,size):

        #
        model = tf.keras.Sequential()

        #
        model.add(tf.keras.layers.Embedding(input_dim = len(lista) + 1, 
                                            output_dim = 128, 
                                            input_shape = (size,), 
    ...
    def cria_stopper_rede(self,
        ...
        return callbacks, history, model

    def feelings_prediction(self,model):
        ...
        text1 = self.preprocess_text(text)

        text1 = [self.vectorize2(text1)]
    ...
    def feelings_prediction_v02(self,model,df):
    
        # organiza os labels pelo valor da rating dado pelo cliente
        dict_feeling = {2: 'Positive', 1: 'Negative', 0: 'Neutral'}

        self.text = df['vectors'].values

        self.text = tf.keras.preprocessing.sequence.pad_sequences(text, maxlen=20)

        self.lista_pred = model.predict(text)
    ...
    def cria_grafico_epochs(history):
