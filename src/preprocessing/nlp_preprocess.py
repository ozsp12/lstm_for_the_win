...
class class_train_model():    
    ...
    def dic_bag_of_worlds(self,lista):
        
        dic_bag = {}
        for word in lista:
            if word not in dic_bag.keys():
                dic_bag[word] = 1
            else:
                dic_bag[word] += 1
    ...
    def df_dic_bag(self,df,coluna):
        ...
        dic_final = self.dic_bag_of_worlds(lista)

        df_bag = pd.DataFrame(list(dic_final.items()),columns=['Palavra', 'Quantidade'])

        df_bag = df_bag.sort_values('Quantidade', ascending=False).reset_index().drop(columns=['index'])
